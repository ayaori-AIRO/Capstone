...
if contours:
    ...
    if is_safe_angle or is_safe_hsv:
        status_text = f"SAFE ({current_angle:.1f} deg)"
        line_color = (0, 255, 0)
    else:
    ...
    print(f"[gauge_debug] 각도 판정: {is_safe_angle}, HSV 판정: {is_safe_hsv}")
...
cv2.imshow("4. Green Mask", green_mask)
cv2.imshow("5. Clean Green", clean_green)
cv2.imshow("6. Edges", edge_display)
cv2.imshow("7. Final Result", img_result)  # 중심-바늘 끝 연결 및 각도 표시
